...
namespace SlaveMatrix
{
    public class 性器_人 : 性器
    {
    	...
    	public 性器_人(double DisUnit, 配色指定 配色指定, 体配色 体配色, Med Med, 性器_人D e)
    	{
    		性器_人 性器_人2 = this;
    		ThisType = GetType();
    		本体 = new Difs(Sta.性器["性器"]);
    		Pars pars = 本体[0][0];
    		...
    		if (e.陰核_接続.Count > 0)
    		{
    			陰核_接続 = e.陰核_接続.Select(delegate(EleD g)
    			{
    				...
    				f.Par = 性器_人2;
    				f.ConnectionType = ConnectionInfo.性器_人_陰核_接続;
    				f.接続(性器_人2.陰核_接続点);
    				return f;
    			}).ToArray();
    		}
    		if (e.尿道_接続.Count > 0)
    		{
    			尿道_接続 = e.尿道_接続.Select(delegate(EleD g)
    			{
    				f = g.GetEle(DisUnit, Med, 体配色);
    				f.Par = 性器_人2;
    				f.ConnectionType = ConnectionInfo.性器_人_尿道_接続;
    				f.接続(性器_人2.尿道_接続点);
    				return f;
    			}).ToArray();
    		}
    		if (e.膣口_接続.Count > 0)
    		{
    			膣口_接続 = e.膣口_接続.Select(delegate(EleD g)
    			{
    				f = g.GetEle(DisUnit, Med, 体配色);
    				f.Par = 性器_人2;
    				f.ConnectionType = ConnectionInfo.性器_人_膣口_接続;
    				f.接続(性器_人2.膣口_接続点);
    				return f;
    			}).ToArray();
    		}
