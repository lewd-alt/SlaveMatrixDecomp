...
namespace SlaveMatrix
{
    public class 双目 : Element
    {
    	...
    	public 双目(double DisUnit, 配色指定 配色指定, BodyColorSet 体配色, ModeEventDispatcher Med, 双目D e)
    	{
    		双目 双目2 = this;
    		ThisType = GetType();
    		Body = new VariantGrid(GlobalState.胴体["EyeLeft"]);
    		...
    		if (e.瞼_接続.Count > 0)
    		{
    			...
    			瞼_接続 = e.瞼_接続.Select(delegate(ElementData g)
    			{
    				...
    				f.Par = 双目2;
    				f.ConnectionType = ConnectionInfo.双目_瞼_接続;
    				f.接続(双目2.瞼_接続点);
    				return f;
    			}).ToArray();
    		}
