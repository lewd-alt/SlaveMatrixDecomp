...
public class 前髪_三分1 : 前髪
{
	...
	public double 髪長
	{
		set
		{
			...
			X0Y0_髪右6.SizeYBase *= num;
			X0Y0_髪左6.SizeYBase *= num;
			X0Y0_髪頭頂横右1.SizeYBase *= num;
			X0Y0_髪頭頂横右2.SizeYBase *= num;
		}
	}

	public 前髪_三分1(double DisUnit, 配色指定 配色指定, 体配色 体配色, Med Med, 前髪_三分1D e)
	{
		ThisType = GetType();
		Dif dif = new Dif();
		dif.Tag = "三分1";
		dif.Add(new Pars(Sta.胴体["前髪"][0][7]));
		本体 = new Difs();
		本体.Tag = dif.Tag;
		本体.Add(dif);
		Pars pars = 本体[0][0];
