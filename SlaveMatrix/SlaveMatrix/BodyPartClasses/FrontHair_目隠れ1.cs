...
{
    public class FrontHair_目隠れ1 : FrontHair
    {
    	...
    	public double 髪長
    	{
    		set
    		{
    			...
    			X0Y0_髪右7.SetSizeYBase(X0Y0_髪右7.GetSizeYBase() * num);
    			X0Y0_ハイライト左.SetSizeYBase(X0Y0_ハイライト左.GetSizeYBase() * num);
    			X0Y0_ハイライト右.SetSizeYBase(X0Y0_ハイライト右.GetSizeYBase() * num);
    		}
    	}

    	public FrontHair_目隠れ1(double DisUnit, 配色指定 配色指定, BodyColorSet 体配色, ModeEventDispatcher Med, FrontHair_目隠れ1D e)
    	{
    		ThisType = GetType();
    		MorphVariant morphVariant = new MorphVariant();
    		morphVariant.Tag = "目隠れ1";
    		morphVariant.Add(new PartGroup(GlobalState.胴体["FrontHair"][0][17]));
    		...
    		X0Y0_髪右7CP = new ColorP(X0Y0_髪右7, 髪右7CD, DisUnit, abj: false);
